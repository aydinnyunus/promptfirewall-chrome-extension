  ...
  document.getElementById("save").addEventListener("click", function () {
    ...
    if (!userInput) {
      messageElement.textContent =
        "Input cannot be empty. Please enter a valid key.";
        ...
        if (response.words && response.words.length > 0) {
          response.words.forEach(function (word) {
            lst.append(
              "<li>" +
                word.word +
                ' <button class="deleteWord" data-id="' +
                word.id +
                '">Delete</button></li>'
            );
          });
            ...
            if (response.message === "Word successfully added") {
              chrome.runtime.sendMessage(
                ...
                function (response) {
                  if (response.words && response.words.length > 0) {
                    var lastAddedWord =
                      response.words[response.words.length - 1];
                    lst.append(
                      "<li>" +
                        lastAddedWord.word +
                        ' <button class="deleteWord" data-id="' +
